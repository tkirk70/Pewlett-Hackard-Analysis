...
SELECT AVG(DATE_PART('year', '2022-01-10'::date) - DATE_PART('year', e.birth_date::date)) AS avg_age
FROM employees e
JOIN unique_titles u
ON u.emp_no = e.emp_no;
...
AVG(DATE_PART('year', '2022-01-10'::date) - DATE_PART('year', e.birth_date::date))
AS avg_age
FROM titles t
JOIN employees e
